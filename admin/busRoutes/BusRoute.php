<?php
    ...
    class BusRoute{
        ...
        $boardingTime,$droppingPoint,$droppingTime,$fare,$totalSeats){
            ...
            $databaseConnect = new DatabaseConnect();
            $conn = $databaseConnect->getInstance();
            $sql = "UPDATE bus_routes SET from_location = ?,to_location = ?,bus_name = ?,bus_type = ?,duration = ?,boarding_point = ?,boarding_time = ?,dropping_point = ?,dropping_time = ?,fare = ?,total_seats = ? WHERE id = ?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("sssssssssiii",$fromLocation,$toLocation,$busName,$busType,$duration,
            $boardingPoint,$boardingTime,$droppingPoint,$droppingTime,$fare,$totalSeats,$routeId);
            $stmt->execute();
